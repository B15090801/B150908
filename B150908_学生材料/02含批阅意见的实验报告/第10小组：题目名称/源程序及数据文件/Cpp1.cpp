...
printf("choose one operation you want to do:\n");
		     ...
		     switch(choice)
		     {
			   ...
			   case 2:  printf("Input the number deleted\n");
					 ...
					 n=deletePla(pla,n,s);   /*调用函数删除指定号码的球员记录*/
					 break;
...
void countManage(Player pla[],int n)               /*该函数完成球员成绩统计功能*/
{
		...
		{
			menuCount( );                        /*显示对应的二级菜单*/
			calcuScore(mark,pla,n);                 /*调用此函数求三个项目的最高、最低*/
			printf("choose one operation you want to do:\n");
			scanf("%d",&choice);
			switch(choice)
			{
...
int runMain(Player pla[],int n,int choice)    /*主控模块，对应于一级菜单其下各功能选择执行*/
{
		switch(choice)
		{
			case 1: printHead( );           /* 1. 显示基本信息*/
				 sortPla(pla,n,1);         /*按球员号码由小到大的顺序排序记录*/
          	 printPla(pla,n);          /*按球员号码由小到大的顺序输出所有记录*/
					break;
...
int main( )
{
	...
	sortPla(pla,n,1);                   /*存入文件前按球员号码由小到大排序*/
